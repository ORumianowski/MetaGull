...
```{r}
comput_param = data.frame(ni=40000,
                          nb=10000,
                          nc=3,
                          nt=40,
                          na=4000)
...
out_info = out1
load(file = "v12_prior_p_hyper2.Rda")
out_non_info = out1
```
...
hyper_no_info = c(NA, NA, out_non_info$mean$phi[2], out_non_info$mean$phi[1], out_non_info$mean$breedingfidelity, out_non_info$mean$natalfidelity, out_non_info$mean$kappa) %>% round(., 3)
```
...
```{r}
main_parameters = data.frame(cbind(estimates, cmr_peron, hyper_info, hyper_no_info,integrated_peron))
```
...
```{r}
eta = out_non_info$mean$eta_monitored %>% round(., 2)
...
```{r}
nu = out_non_info$mean$nu_monitored %>% round(., 2)
...
data$p <- out_non_info$mean$po[6:9,] %>% 
  as.data.frame() %>% 
  t() %>% 
  as.vector()
...
```{r}
MCMCtrace(out_non_info, params = c("mean_p_LR","mean_p_MA","mean_p_VE","mean_p_V5",
...
MCMCtrace(out_non_info, params = c("phi"),
          Rhat = T,
          priors = PR_unif_01,
          gvals = gvals,
          ind = T, pdf = F,
...
MCMCtrace(out_non_info, params = c("kappa"),
          Rhat = TRUE,
          priors = PR_unif_01,
          gvals = gvals,
          ind = TRUE, pdf = FALSE,
...
MCMCtrace(out_non_info, params = c("eta_monitored"),
          Rhat = TRUE,
          #priors = PR_unif_01,
          ind = TRUE, pdf = FALSE,
          post_zm = F)
...
MCMCtrace(out_non_info, params = c("nu_monitored"),
          Rhat = TRUE,
          #priors = PR_unif_01,
          ind = TRUE, pdf = FALSE,
          post_zm = F)
