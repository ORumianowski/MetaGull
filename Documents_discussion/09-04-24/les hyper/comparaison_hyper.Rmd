...
out_2 = out1
load(file = "out1_cmr_hyper_AE2.Rda")
out_3 = out1
```
...
dt_p_1 <- out_1$mean$po[6:9,] %>%
  ...
  setNames(c("LR", "MA", "VE", "V5")) %>%
  mutate(year = 1:nrow(.)) %>%
  mutate(dataset = rep("1-Hyper-Informatif", nrow(.))) %>%
  pivot_longer(cols = -c(year, dataset), 
               names_to = "Type",
               values_to = "Valeur")
...
dt_p_2 <- out_2$mean$po[6:9,] %>%
  ...
  setNames(c("LR", "MA", "VE", "V5")) %>%
  mutate(year = 1:nrow(.)) %>%
  mutate(dataset = rep("2-Hyper-Non-informatif", nrow(.))) %>%
  pivot_longer(cols = -c(year, dataset), 
               names_to = "Type",
               values_to = "Valeur")


dt_p_3 <- out_3$mean$po[6:9,] %>%
  t() %>%
  as.data.frame() %>%
  setNames(c("LR", "MA", "VE", "V5")) %>%
  mutate(year = 1:nrow(.)) %>%
  mutate(dataset = rep("3-Hyper&AEfixé", nrow(.))) %>%
  pivot_longer(cols = -c(year, dataset), 
               names_to = "Type",
               values_to = "Valeur")

dt_p = rbind(dt_p_0, dt_p_1, dt_p_2, dt_p_3) %>% 
  subset(., Valeur != 0)
...
## Hyperparametres cadrés
### Mean and SD

```{r}
MCMCtrace(out_1, params = c("mean_p_LR","mean_p_MA","mean_p_VE","mean_p_V5",
...
## Hyperparametres non-infomratif
