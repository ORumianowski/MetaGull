...
cat(file = "2_models/3_models_for_real_period2/v1/models/cmr_period2_v2_disp.txt", "
model {
 # -------------------------------------------------
  # Stages:
  ...
  for (t in 1:(n.years-1)){
    ...
    for (next_state in breed.states){
        psi[prev.state,next_state,t] <- phi[2,year2period[t]] * kappa[state2colclass[next_state]] * eta_t[state2col[prev.state],state2col[next_state],t]
    }
    
    # Prebreeders may stay prebreeders
    for (next_state in prebr.states){
          psi[prev.state,next_state,t] <- equals(state2col[next_state], state2col[prev.state]) 
          * phi[2,year2period[t]] * sum(eta_t[state2col[prev.state],1:n.colony,t] * (1-kappa[col2colclass]))
...
out1 <- jags(jags.data, inits, parameters, "2_models/3_models_for_real_period2/v1/models/cmr_period2_v2_disp.txt",
