...
cat(file = "2_models/3_models_for_real_period2/v1/models/integrated_period2_all_hyper.txt", "
model {
 # -------------------------------------------------
  # Stages:
  ...
  for (period in 1:nb_period){
    rho[1,period] ~ dunif(0, 3) # Productivity - La Ronze
    rho[2,period] ~ dunif(0, 3) # Productivity - Satellite colonies
  }
  
  for (period in 1:nb_period){
    phi[1,period] ~  dunif(0, 1) # Survival prob. - First year
    phi[2,period]  ~ dunif(0, 1) # Survival prob. - Subadult and adults
  }

  for (period in 1:nb_period){
    kappa[1,period] ~  dunif(0, 1) # Recruitment prob. -  La Ronze
    kappa[2,period] ~ dunif(0, 1) # Recruitment prob. -  Satellite colonies
  }
   ...
   for (period in 1:nb_period){
    for (dep in 1:n.colony){
      for (arr in 1:n.colony){
        eta[dep, arr,period]  ~ dunif(0, 1)   # natal dispersal
        nu[dep, arr,period]  ~ dunif(0, 1)    # breeding dispersal
      }
    }
  }
  ...
  for (period in 1:nb_period){
    for (dep in 1:n.colony){
      for (arr in 1:n.colony){
        eta_monitored[dep, arr, period] <- (eta[dep, arr, period] / sum(eta[dep,,period])) 
        nu_monitored[dep, arr, period] <- (nu[dep, arr, period] / sum(nu[dep,,period])) 
      }
    }
  }
  ...
  for (colony in 1:n.colony){ 
    for (period in 1:nb_period){
      natalfidelity[colony, period] <- eta_monitored[colony,colony, period]
      breedingfidelity[colony, period] <- nu_monitored[colony,colony, period]
    }
    DIFFbreedingfidelity[colony] <- breedingfidelity[colony, 1] - breedingfidelity[colony, 3]
  }
    
  for (t in 1:(n.years-1)){
    for (dep in 1:n.colony){
      for (arr in 1:n.colony){
          eta_ext[dep, arr, t] <-  eta[dep, arr, year2period[t]]  * (1-equals(E[arr,(t+1)],0)) # The value is 0 if you are moving towards a colony that has disappeared
          nu_ext[dep, arr, t] <- nu[dep, arr, year2period[t]] * (1-equals(E[arr,(t+1)],0)) # The value is 0 if you are moving towards a colony that has disappeared
      }
    }
  }
  ...
  for (t in 1:(n.years-1)){
    ...
    N[s,t+1] <- B[s,t] * rho[col2colclass[s],year2period[t]] * phi[1,year2period[t]] +
                N[s,t] * phi[2,year2period[t]] * sum(eta_t[s,1:n.colony,t] * (1-kappa[col2colclass,year2period[t]]))

    B[s,t+1] <- sum(N[1:n.colony,t] * phi[2,year2period[t]] * eta_t[1:n.colony,s,t]  * kappa[col2colclass[s],year2period[t]] ) +
                sum(B[1:n.colony,t] * phi[2,year2period[t]] * nu_t[1:n.colony,s,t])
    }
  }

  # Residual (observation) error

  sigma0[1] ~ dunif(0.01, 1000) # La Ronze - Period 1
  sigma0[2] ~ dunif(0.01, 1000) # La Ronze - Period 2 & 3
  sigma0[3] ~ dunif(0.01, 1000) # Satellite colonies - Period 1 & 2 & 3
  ...
  for (t in 1:(n.years)){
    ...
    R[s,t] <- sigma[col2colclass[s],year2period[t]] * B[s,t]* (1-equals(E[s,t],0)) + 1E-9
    tau[s,t] <- pow(R[s,t], -2)
  ...
  for (t in 1:(n.years-1)){
    ...
    for (prev.state in nest.states){
      ...
      for (next_state in prebr.states){
        psi[prev.state,next_state,t] <- phi[1,year2period[t]] * equals(state2col[next_state], state2col[prev.state]) 
      }
    ...
    for (prev.state in breed.states){
      ...
      for (next_state in breed.states){
        psi[prev.state,next_state,t] <- phi[2,year2period[t]] * nu_t[state2col[prev.state],state2col[next_state],t]
      }
    ...
    for (next_state in breed.states){
        psi[prev.state,next_state,t] <- phi[2,year2period[t]] * kappa[state2colclass[next_state],year2period[t]] * eta_t[state2col[prev.state],state2col[next_state],t]
    }
    
    # Prebreeders may stay prebreeders
    for (next_state in prebr.states){
          psi[prev.state,next_state,t] <- equals(state2col[next_state], state2col[prev.state]) 
          * phi[2,year2period[t]] * sum(eta_t[state2col[prev.state],1:n.colony,t] * (1-kappa[col2colclass,year2period[t]]))
...
out1 <- jags(jags.data, inits, parameters, "2_models/3_models_for_real_period2/v1/models/integrated_period2_all_hyper.txt",
