...
# load(file = "/lustre/rumianowskio/simulated_dataset_050424/B.Rda")
# load(file = "/lustre/rumianowskio/simulated_dataset_050424/marr.Rda")
...
cat(file = "2_models/1_models_for_simulation/models/integrated_v14_3_2.txt", "
model {
 # -------------------------------------------------
  # Stages:
  ...
  for (dep in 1:n.colony) {
    ...
    nu[dep, 1:n.colony]  ~ ddirch(dirich_param)  # breeding dispersal
        for (arr in 1:n.colony){
          eta_real[dep, arr] ~ dnorm(eta[dep, arr], 100000000)  # breeding dispersal
      }
  }
  ...
  for (t in 1:(n.years - 1)) {
   # Surveyed colonies/years - breeders
    for (state in ((n.colony+1):(2*n.colony-1))) {
      lpo[state, t] ~ dnorm(mup[state-n.colony], taup[state-n.colony])
      po[state, t]  <- (exp(lpo[state, t]) / (1 + exp(lpo[state, t])))
      }
  }
...
ni <- 50000; nb <- 10000; nc <- 3; nt <- 70; na <- 4000


# Call JAGS from R and check convergence
out1 <- jags(jags.data, inits, parameters, "2_models/1_models_for_simulation/models/integrated_v14_3_2.txt",
