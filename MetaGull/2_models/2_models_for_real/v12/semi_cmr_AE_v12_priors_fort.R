...
cat(file = "2_models/2_models_for_real/v12/models/semi_cmr_AE_v12_priors_fort.txt", "
model {
 # -------------------------------------------------
  # Stages:
  ...
  phi[1] ~  dunif(0.273*0.9, 0.273*1.1) # Survival prob. - First year
  phi[2]  ~ dunif(0.828*0.9, 0.828*1.1) # Survival prob. - Subadult and adults
  
  kappa[1] ~ dunif(0.716*0.9, 0.716*1.1) # Recruitment prob. -  La Ronze
  kappa[2] ~ dunif(0.827*0.9, 0.827*1.1) # Recruitment prob. -  Satellite colonies
  ...
  for (k in 1:nrow_detection_index) {
    po[detection_index[k, 1], detection_index[k, 2]] ~ dunif(0, 0.7)
  }
...
out1 <- jags(jags.data, inits, parameters, "2_models/2_models_for_real/v12/models/semi_cmr_AE_v12_priors_fort.txt",
