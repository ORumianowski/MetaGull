...
cat(file = "2_models/2_models_for_real/v12/models/semi_cmr_AE_v12_2.txt", "
model {
 # -------------------------------------------------
  # Stages:
  ...
  sigma2_mean <- 0.1
  sigma2_precision ~ dunif(10, 100)
  
  sigma[2] ~ dnorm(sigma2_mean, sigma2_precision) # Satellite colonies
...
out1 <- jags(jags.data, inits, parameters, "2_models/2_models_for_real/v12/models/semi_cmr_AE_v12_2.txt",
             n.iter=ni, n.burnin=nb, n.chains=nc, n.thin=nt, n.adapt=na,
             parallel=TRUE)

save(out1, file = "/lustre/rumianowskio/out1_cmr_AE_v12_2.Rda")
