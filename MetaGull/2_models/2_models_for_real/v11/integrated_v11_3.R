...
repro_ratio = c(0.4/0.6, rep(0.3/0.7,n.colony-1))  # Age ratio estimated with a formula from Kery 

width = 0.6

pop_init = data.frame(estim = B[,1]) %>% 
  mutate(B_inf = estim*(1-width))%>% 
  mutate(B_sup = estim*(1+width)+1)%>% 
  mutate(N_inf = estim*(repro_ratio)*(1-width))%>% 
  mutate(N_sup = estim*(repro_ratio)*(1+width)+1) %>% 
  round() %>% 
  as.matrix()
...
cat(file = "2_models/2_models_for_real/v11/models/model_v11_2.txt", "
model {
 # -------------------------------------------------
  # Stages:
  ...
  phi[1] ~  dunif(0, 1) # dnorm(0.22, 1000000) # dunif(0.21, 0.23) # Survival prob. - First year
  phi[2]  ~ dunif(0, 1) # dnorm(0.86, 1000000) # dunif(0, 1) # <- 0.86 # ~    # Survival prob. - Subadult and adults
...
#   #B[1,1] <- rpois(1, pop_init[1,1]+1)
#   #B[2,1] <- rpois(1, pop_init[2,1]+1)
#   #B[3,1] <- rpois(1, pop_init[3,1]+1)
#   #B[4,1] <- rpois(1, pop_init[4,1]+1)
#   #B[4,1] <- rpois(1, pop_init[5,1]+1)
# 
#   #N <- B * 0.35/0.65
#   
#   phi <- array(NA, dim=c(2))
#   phi[1] <- runif(1, 0.21, 0.22)
#   phi[2] <- runif(1, 0.85, 0.87)
#   
#   rho <- array(NA, dim=c(2))
#   rho[1] <- runif(1, 1.7, 1.8)
#   rho[2] <- runif(1, 0.6, 0.7)
#   
#   kappa <- array(NA, dim=c(2))
#   kappa[1] <- runif(1, 0.6, 0.62)
#   kappa[2] <- runif(1, 0.84, 0.85)
#   
#     
#   return(list(phi=phi, rho=rho, kappa=kappa))
# }
...
out1 <- jags(jags.data, inits, parameters, "2_models/2_models_for_real/v11/models/model_v11_2.txt",
             n.iter=ni, n.burnin=nb, n.chains=nc, n.thin=nt, n.adapt=na,
             parallel=TRUE)

save(out1, file = "/lustre/rumianowskio/out1_integrated_v10_fidelity_v3.Rda")
