...
load(file = "real_dataset/B.Rda")
load(file = "real_dataset/marr.Rda")

# load(file = "real_dataset_v2/CH.Rda")
# CH = matrix(CH %>% as.integer(), ncol = ncol(CH), nrow = nrow(CH))
# 
...
real_eta = c(0.6778150, 0.12588661, 0.1656414, 0.02388027, 0.006776811,
              0.1378559, 0.36926837, 0.3430739, 0.05585749, 0.093944317,
              0.1335643, 0.23941253, 0.4437549, 0.14973137, 0.033536913,
              0.2246525, 0.13267355, 0.1740707, 0.27608597, 0.192517264,
              0.3786668, 0.06450448, 0.0920614, 0.12790852, 0.336858762) %>% matrix(., ncol = 5, nrow = 5, byrow = T )

real_nu =c( 0.96846136, 0.003130055, 0.007626411, 0.009943757, 0.01083842,
 0.08393526, 0.580836593, 0.195441797, 0.026228494, 0.11355786,
 0.08626791, 0.135002641, 0.489236785, 0.114183560, 0.17530910,
 0.22546190, 0.116062717, 0.093187365, 0.510884971, 0.05440305,
 0.09797417, 0.022704103, 0.117656571, 0.015507490, 0.74615767) %>% matrix(., ncol = 5, nrow = 5, byrow = T )
...
model {
 # -------------------------------------------------
  ...
  phi[1] ~ dunif(0.21, 0.215) # Survival prob. - First year
  phi[2] ~ dunif(0.85, 0.87) # Survival prob. - Subadult and adults
  
  kappa[1] ~ dunif(0.61, 0.63) # Recruitment prob. -  La Ronze
  kappa[2] ~ dunif(0.84, 0.86) # Recruitment prob. -  Satellite colonies
  ...
  for (dep in 1:n.colony){
    for (arr in 1:n.colony){
      eta[dep, arr]  <- real_eta[dep, arr]   # natal dispersal
      nu[dep, arr]  <- real_nu[dep, arr]   # breeding dispersal
  ...
  #     po[state,t] <- 0
  #   }
  #   for (state in (n.colony+1):(2*n.colony-1)){ #  breed.states){
  #     po[state,t] ~ dunif(0, 1)
  #   }
  #   po[2*n.colony,t] <- 0
  # 
...
ni <- 20000; nb <- 3000; nc <- 1; nt <- 40; na <- 1000


# Call JAGS from R and check convergence
out1 <- jags(jags.data, inits, parameters, "model_v10_real_A.txt",
             n.iter=ni, n.burnin=nb, n.chains=nc, n.thin=nt, n.adapt=na,
             parallel=TRUE)

#save(out1, file = "/lustre/rumianowskio/out1_integrated_v8_2.Rda")


library(MCMCvis)
MCMCtrace(out1, params = c("rho"), ind = TRUE, pdf = FALSE)
