...
cat(file = "2_models/2_models_for_real/model_v10_7_3_2.txt", "
model {
 # -------------------------------------------------
  # Stages:
  ...
  rho[1] ~ dunif(1.7, 1.75) # Productivity - La Ronze
  rho[2] ~ dunif(0.6, 0.61) # Productivity - Satellite colonies
...
out1 <- jags(jags.data, inits, parameters, "2_models/2_models_for_real/model_v10_7_3_2.txt",
             n.iter=ni, n.burnin=nb, n.chains=nc, n.thin=nt, n.adapt=na,
             parallel=TRUE)

save(out1, file = "/lustre/rumianowskio/out1_integrated_v10_7_3_2_real.Rda")
