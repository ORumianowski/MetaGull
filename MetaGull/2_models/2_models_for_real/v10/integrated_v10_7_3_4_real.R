...
cat(file = "2_models/2_models_for_real/model_v10_7_3_4.txt", "
model {
 # -------------------------------------------------
  # Stages:
  ...
  eta[n.colony, 1] <- eta[2, 1] + eta[3, 1] + eta[4, 1] 
  eta[n.colony, 2] <- eta[1, 2] + eta[3, 2] + eta[4, 2] 
  eta[n.colony, 3] <- eta[1, 3] + eta[2, 3] + eta[4, 3] 
  eta[n.colony, 4] <- eta[1, 4] + eta[2, 4] + eta[3, 4] 
  eta[n.colony, 5] ~ dunif(0, 1)
  
  nu[n.colony, 1] <- nu[2, 1] + nu[3, 1] + nu[4, 1] 
  nu[n.colony, 2] <- nu[1, 2] + nu[3, 2] + nu[4, 2] 
  nu[n.colony, 3] <- nu[1, 3] + nu[2, 3] + nu[4, 3] 
  nu[n.colony, 4] <- nu[1, 4] + nu[2, 4] + nu[3, 4] 
  nu[n.colony, 5] ~ dunif(0, 1)
...
out1 <- jags(jags.data, inits, parameters, "2_models/2_models_for_real/model_v10_7_3_4.txt",
             n.iter=ni, n.burnin=nb, n.chains=nc, n.thin=nt, n.adapt=na,
             parallel=TRUE)

save(out1, file = "/lustre/rumianowskio/out1_integrated_v10_7_3_4_real.Rda")
