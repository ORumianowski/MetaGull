...
jags.data <- list(C=survey_data,
                  ...
                  marr_index_with_cmr2 = marr_index_with_cmr[!(marr_index_with_cmr %in% c(96, 97, 100))],
...
cat(file = "model_v10_7_3_survival.txt", "
model {
 # -------------------------------------------------
  # Stages:
  ...
  phi[1] <- 0.213 # ~ dunif(0, 1) # dunif(0.21, 0.23) # Survival prob. - First year
  phi[2]  <- 0.86 # dnorm(0.86, 0.1)  #dunif(0, 1) # <- 0.86 # ~   dunif(0.84, 0.88) # Survival prob. - Subadult and adults
...
out1 <- jags(jags.data, inits, parameters, "2_models/2_models_for_real/model_v10_7_3_survival.txt",
             n.iter=ni, n.burnin=nb, n.chains=nc, n.thin=nt, n.adapt=na,
             parallel=TRUE)

save(out1, file = "/lustre/rumianowskio/out1_integrated_v10_7_3_real.Rda")
