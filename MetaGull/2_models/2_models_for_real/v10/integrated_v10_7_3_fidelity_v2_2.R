...
cat(file = "2_models/2_models_for_real/model_v10_7_3_fidelity_v2_2.txt", "
model {
 # -------------------------------------------------
  # Stages:
  ...
  eta_monitored[n.colony, 1] <- (eta[n.colony, 1] / sum(eta[n.colony,])) 
  eta_monitored[n.colony, 2] <- (eta[n.colony, 2] / sum(eta[n.colony,])) 
  eta_monitored[n.colony, 3] <- (eta[n.colony, 3] / sum(eta[n.colony,])) 
  eta_monitored[n.colony, 4] <- (eta[n.colony, 4] / sum(eta[n.colony,]))  
  eta_monitored[n.colony, 5] ~ dunif(0.3, 0.31)
  
  nu_monitored[n.colony, 1] <- (nu[n.colony, 1] / sum(nu[n.colony,])) 
  nu_monitored[n.colony, 2] <- (nu[n.colony, 2] / sum(nu[n.colony,])) 
  nu_monitored[n.colony, 3] <- (nu[n.colony, 3] / sum(nu[n.colony,])) 
  nu_monitored[n.colony, 4] <- (nu[n.colony, 4] / sum(nu[n.colony,])) 
  nu_monitored[n.colony, 5] ~ dunif(0.93, 0.94)
...
ni <- 30000; nb <- 10000; nc <- 3; nt <- 60; na <- 4000


# Call JAGS from R and check convergence
out1 <- jags(jags.data, inits, parameters, "2_models/2_models_for_real/model_v10_7_3_fidelity_v2_2.txt",
             n.iter=ni, n.burnin=nb, n.chains=nc, n.thin=nt, n.adapt=na,
             parallel=TRUE)

save(out1, file = "/lustre/rumianowskio/out1_integrated_v10_fidelity_v2_2.Rda")
