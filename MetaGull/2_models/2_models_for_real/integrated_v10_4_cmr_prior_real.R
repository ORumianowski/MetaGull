...
cat(file = "model_v10_4_cmr_prior.txt", "
model {
 # -------------------------------------------------
  # Stages:
  ...
  phi[1] ~ dunif(0.2, 0.35) # Survival prob. - First year
...
ni <- 30000; nb <- 10000; nc <- 3; nt <- 40; na <- 4000


# Call JAGS from R and check convergence
out1 <- jags(jags.data, inits, parameters, "model_v10_4_cmr_prior.txt",
             n.iter=ni, n.burnin=nb, n.chains=nc, n.thin=nt, n.adapt=na,
             parallel=TRUE)

save(out1, file = "/lustre/rumianowskio/out1_integrated_v10_4_cmr_prior_real.Rda")
