...
model {
 # -------------------------------------------------
  ...
  for (k in 1:nrow_no_detection_index) {
    po[no_detection_index[k, 2] + n.colony, no_detection_index[k, 1]] <- 0 #  ~  dunif(0, 1) # 
  }
...
out1 <- jags(jags.data, inits, parameters, "model_v10_5_cmr.txt",
